...
START_TEST(test_create_account_null_channel)
{
    ...
    ck_assert_msg(createAccount(channel, username, accountID) == FAILURE,
                  "createAccount should return FAILURE when channel is NULL");
}
END_TEST
...
START_TEST(test_create_account_null_username)
{
    ...
    ck_assert_msg(createAccount(channel, username, accountID) == FAILURE,
                  "createAccount should return FAILURE when username is NULL");
...
START_TEST(test_create_account_from_empty)
{
    ...
    ck_assert_msg(createAccount(channel, username, accountID) != FAILURE,
                  "createAccount should not return FAIlURE with valid username and valid channel");
...
START_TEST(test_create_account_malloc_failure)
{
    ...
    // createAccount requires at least 3 malloc calls to be successful
...
START_TEST(test_create_account_realloc_failure)
{
    ...
    mallocs_until_fail = 2;
    ck_assert_msg(createAccount(c, "pgupta359", 3) == FAILURE,
                  "createAccount should return FAILURE whenever malloc fails");
    _compare_channels(expected_c, c, "createAccount should not alter the channel: ");

    mallocs_until_fail = -1;
    ck_assert_msg(createAccount(c, "pgupta359", 3) == SUCCESS,
                  "createAccount should be successful if malloc does not fail");
    _create_account(expected_c, "pgupta359", 3);
    _compare_channels(expected_c, c, "");

    // try again with another account to make sure they realloc again
    mallocs_until_fail = 2;
    ck_assert_msg(createAccount(c, "rbafna3", 4) == FAILURE,
                  "createAccount should return FAILURE whenever malloc fails");
    mallocs_until_fail = -1;
    ck_assert_msg(createAccount(c, "rbafna3", 4) == SUCCESS,
                  "createAccount should be successful if malloc does not fail");
...
START_TEST(test_create_account_no_duplicates_short)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        ck_assert_msg(SUCCESS == actual_result,
                      "createAccount should return SUCCESS after successfully creating a new account");

        _create_account(expected, usernames[i], ids[i]);
...
START_TEST(test_create_account_no_duplicates_long)
{
    ...
    // Add Accounts to channel
    // Create Accounts first
    char *usernames[17] = {"Justin", "Jeff", "Richard", "Marto", "Sophie", "Gal", "Sarah", "Saloni", "Alex",
                           "Annelise", "Darren", "Gilbert", "Dan", "Aaron", "Lydia", "Prabhav", "Heather"};
...
START_TEST(test_create_account_duplicates_short)
{
    ...
    // Add Accounts to channel
    // Create Accounts first
    char *usernames[4] = {"Richard", "Richard", "Rohan", "Greg"};
    int ids[4] = {0, 0, 0, 2110};

    int expected_retvals[4] = {SUCCESS, FAILURE, FAILURE, SUCCESS};
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        ck_assert_msg(
            actual_result == expected_retvals[i],
            expected_retvals[i] == SUCCESS
                ? "createAccount should return SUCCESS after successfully creating a new account"
                : "createAccount should return FAILURE when trying to create an account with a duplicate accountID");
...
START_TEST(test_create_account_duplicates_long)
{
    ...
    // Add Accounts to channel
    // Create Accounts first
    char *usernames[17] = {"Justin", "Jeff", "Justin", "Marto", "Sophie", "Gal", "Sophie", "Saloni", "Alex",
                           "Alex", "Darren", "Gilbert", "Dan", "Justin", "Lydia", "Prabhav", "Heather"};
...
START_TEST(test_add_reaction_from_empty)
{
    ...
    ck_assert_msg(!addReaction(c->posts.head->data, 1, THUMBS_UP),
                  "addReaction should return SUCCESS if args are valid");
    _compare_channels(expected_c, c, "");
    _free_channel(c);
    _free_channel(expected_c);
}
...
START_TEST(test_add_reaction_multiple_posts)
{
    ...
    Post posts[] = {{0, 0, "hello, world", {{0, SMILEY}, {1, SKULL}}, 2},
...
START_TEST(test_add_reaction_invalid_arg_checks)
{
    ...
    ck_assert_msg(addReaction(NULL, 0, SKULL) == FAILURE,
                  "addReaction() should return FAILURE when given NULL pointer as post");
...
START_TEST(test_add_reaction_duplicate_accountid)
{
    ...
    ck_assert_msg(addReaction(c->posts.head->data, 1, SKULL) == FAILURE,
                  "addReaction() should return FAILURE when a reaction with the same accountID already exists");
...
START_TEST(test_add_reaction_array_limit)
{
    ...
    ck_assert_msg(addReaction(c->posts.head->data, 8, THUMBS_UP) == FAILURE,
                  "addReaction() should return FAILURE when reactions array is already at max capacity");
...
START_TEST(test_search_palindrome_spaces_specials)
{
    ...
    Post posts[] = {{0, 0, "hello, world", {{0}}, 0},
                    {2, 0, "not a palindrome", {{0}}, 0},
...
START_TEST(test_delete_reaction_basic)
{
    ...
    ck_assert_msg(deleteReaction(c->posts.head->data, 0, SKULL) == SUCCESS,
                  "deleteReaction should return SUCCESS if the reaction is found");
    _compare_channels(expected_c, c, "");
    _free_channel(c);
    _free_channel(expected_c);
}
...
START_TEST(test_delete_reaction_multiple_posts)
{
    ...
    Post posts[] = {{0, 0, "hello, world", {{0, SMILEY}, {1, SKULL}, {3, THUMBS_UP}}, 3},
                    {1, 0, "become a TA!", {{5, SKULL}, {1, THUMBS_UP}, {0, SKULL}, {2, SMILEY}, {3, HEART}}, 5},
                    {2, 0, "hw9 best hw", {{1, SKULL}}, 1}};
    int numPosts = 3;
    _add_posts_to_channel(c, posts, numPosts);

    Post posts_expected[] = {{0, 0, "hello, world", {{1, SKULL}}, 1},
    ...
    ck_assert_msg(!deleteReaction(post1, 3, THUMBS_UP),
                  "deleteReaction should return SUCCESS if the reaction is found");
...
START_TEST(test_delete_reaction_invalid_arg_checks)
{
    ...
    ck_assert_msg(deleteReaction(NULL, 0, SKULL) == FAILURE,
                  "deleteReaction() should return FAILURE when given NULL pointer as post");
...
START_TEST(test_delete_reaction_not_found)
{
    ...
    ck_assert_msg(deleteReaction(c->posts.head->data, 2, HEART) == FAILURE,
                  "deleteReaction() should return FAILURE when the reaction is not found");
    ck_assert_msg(deleteReaction(c->posts.head->data, 1, SKULL) == FAILURE,
                  "deleteReaction() should return FAILURE when the reaction is not found (check against BOTH reaction "
                  "and accountID)");
...
START_TEST(test_delete_account_not_found)
{
    ...
    ck_assert_msg(deleteAccount(channel, 32) == FAILURE,
                  "deleteAccount should return FAILURE when Account is not found");
...
START_TEST(test_delete_channel_full)
{
    ...
    Post posts[] = {{0, 0, "hello, world", {{1, SKULL}, {2, SMILEY}}, 2},
...
START_TEST(test_create_post_null_text)
{
    ...
    ck_assert_msg(createPost(c, NULL, 10, 20) == FAILURE, "createPost should return FAILURE when text is NULL");
    _compare_channels(gt, c, "createPost should not modify channel on failure: ");
    _free_channel(c);
    _free_channel(gt);
}
END_TEST
...
START_TEST(test_create_post_postid_not_unique)
{
    ...
    ck_assert_msg(createPost(c, "Polo!", 2, 40) == FAILURE, "createPost should return FAILURE when postID is not "
                                                            "unique");
    _compare_channels(gt, c, "createPost should not modify channel on failure: ");
    _free_channel(c);
    _free_channel(gt);
...
START_TEST(test_create_post_malloc_failure)
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        ck_assert_msg(createPost(c, msg, 1, 1) == FAILURE, "createPost should return FAILURE when malloc fails");
        _compare_channels(gt, c, "createPost should not modify channel on failure: ");
    }
...
START_TEST(test_create_first_post)
{
    ...
    ck_assert_msg(createPost(c, msg, 1, 1) == SUCCESS, "createPost should return SUCCESS on success");
    _compare_channels(gt, c, "createPost returned SUCCESS, but: ");
    // Make sure the string was actually malloced.
    ck_assert_msg(c->posts.head->data->text != msg, "createPost did not copy the post text");
...
START_TEST(test_create_many_posts)
{
    ...
    for (int i = 0; i < 6; ++i)
    {
        ...
        ck_assert_msg(ret == SUCCESS, "createPost should return SUCCESS on success");
        _add_post_to_end_of_channel(gt, posts + i);
        _compare_channels(gt, c, "createPost returned SUCCESS, but: ");
        ck_assert_msg(c->posts.head->data->text != posts[i].text, "createPost did not copy the post text");
    }
...
{
    ck_assert_msg(!findPost(NULL, 1), "findPost should return NULL if channel is NULL");
}
END_TEST
...
START_TEST(test_find_post_not_found)
{
    ...
    ck_assert_msg(!findPost(c, 5), "findPost should return NULL if no post with id "
                                   "postID exists");
...
START_TEST(test_find_post)
{
    ...
    char m[400];
    snprintf(m, 400, "findPost found post %p (id %d), expected post %p (id %d)", (void *)found_post, found_post->postID,
...
{
    ck_assert_msg(deletePost(NULL, 1) == FAILURE, "deletePost should return failure when channel is NULL");
}
END_TEST
...
START_TEST(test_delete_post_no_posts)
{
    ...
    ck_assert_msg(deletePost(c, 5) == FAILURE, "deletePost should return FAILURE if no post with id "
                                               "postID exists");
    _compare_channels(gt, c, "deletePost should not modify channel on failure: ");
...
START_TEST(test_delete_first_post)
{
    ...
    ck_assert_msg(deletePost(c, 1) == SUCCESS, "deletePost should return SUCCESS on success");
    _compare_channels(gt, c, "deletePost should have deleted post with id 1, but: ");
...
START_TEST(test_delete_middle_post)
{
    ...
    ck_assert_msg(deletePost(c, 10) == SUCCESS, "deletePost should return SUCCESS on success");
    _compare_channels(gt, c, "deletePost should have deleted post with id 10, but: ");
...
START_TEST(test_delete_last_post)
{
    ...
    ck_assert_msg(deletePost(c, 4) == SUCCESS, "deletePost should return SUCCESS on success");
    _compare_channels(gt, c, "deletePost should have deleted post with id 4, but: ");
